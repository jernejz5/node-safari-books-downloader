...
program
  ...
  .option('-c, --cookie <cookie>','cookie token of the SafariBooksOnline user')
  .option('-o, --output <output>','output path the epub file should be saved to')
  .parse(process.argv);

var username, password, cookie;
...
// ## see whether .credentials already exists
  if (fs.existsSync(__dirname + '/.credentials')) {
    ...
    debug("there is an existing user credentials cached");
}
...
if(program.cookie) cookie = program.cookie;
...
let json = JSON.stringify({ "username": username, "password": password, "cookie": cookie });
fs.writeFile(__dirname + '/.credentials', json).then( () => {
  debug(`credentials were successfully cached`);
}).catch( (err) => {
  debug(`an error occurred trying to write credentials to cache file`);
});

// ## Authorize User
var safariClient = new Safari();
safariClient.fetchBookById(program.bookid, username, password, cookie).then( (bookJSON) => {
