...
* @param {String} cookie - the cookie token of the safaribooksonline account
*
* @return {String} accessToken - returns the access token
*/
Safari.prototype.authorizeUser = function authorizeUser(username, password, cookie) {
  if(cookie) {
    this.cookie = cookie;
...
Safari.prototype.fetchResource = function fetchResource(url, options) {
  ...
  if(!this.cookie && !this.accessToken) return Promise.reject("credentials not specified");
...
* @param {String} cookie - the cookie token of the safaribook user
*
* @return {Object} body - returns the book json
*/
Safari.prototype.fetchBookById = function fetchBookById(id, username, password, cookie) {
  debug(`fetchBookById called with id: ${id}`);
  if(!id) return Promise.reject("id was not specified");
  if(!cookie && (!username || !password)) return Promise.reject("credentials not specified");
  // ## validation successful
  // ## fetch required content
  return this.authorizeUser(username, password, cookie).then( (accessToken) => {
